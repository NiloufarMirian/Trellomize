...
cipher = Fernet(key)
panel = Panel(Text("Welcome To The User Management System"))
console = Console()
text = Text()
...
def load_data(filename='data.json'):
    try:
        with open(filename, 'r') as file:
            ...
            projects = [Project.from_dict(project)
...
def register_user(username, email, password):
    ...
    if not is_valid_username(username):
        rprint(
            "Invalid username. It must be alphanumeric and between 3 and 20 characters.")
        return False
...
def create_project(username):
    ...
    id = uuid.uuid4()
    logger.info(f"{username} created project")
...
def display_tasks_by_status(tasks, status):
    ...
    for task in tasks:
        if task.status == status:
            table.add_row(str(task.id), task.title, task.desc,
                          str(task.start), str(task.end))
...
def manage_project_tasks(project, tasks):
    while True:
        ...
        if choice == "1":
            ...
            task = next(
                (task for task in tasks if str(task.id) == task_id), None)
            if task:
